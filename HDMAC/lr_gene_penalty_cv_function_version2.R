...
lr.gene.penalty.cv.result = function(user.data, response.col, gene.type, 
  ...
  lr.gene.penalty.cv.list = sapply(1:nfold, function(fold){
    ...
    if( padj.method == "fdr" ){
      ...
      if (length(fdr.genes.col) == 0) stop("small fdr.threshold --> no genes selected")}else{
    ...
    if( ! adaptive.lasso ){
      ...
      if( nfold == 1 && penalty.alpha == 1 ){
        ...
        li = fixedLassoInf(x[,-which(duplicated(x, MARGIN = 2))], y = y, beta = best_coef[-which(duplicated(x, MARGIN = 2))], lambda = lambda_min, family = "binomial")
